 ...
 *  Pager        [data-dt-pager] nav with first/prev/number/next/last,
 *              ellipsis, summary, page-size select; dispatches
 *              dt:page-change{page,skip,top,pageCount,pageSize} and
 *              dt:page-size-change{pageSize}
 *  Menu         [data-dt-menu] menubar with submenus, hover/click, arrows;
 *              dispatches dt:menu-click{ text, value, path }
 *  PanelMenu    [data-dt-panelmenu] accordion nav, expand/collapse, arrows;
 *              dispatches dt:panelmenu-click{ text, value, path }
 *  ProfileMenu  [data-dt-profilemenu] trigger + dropdown menu, arrows/Escape;
 *              dispatches dt:profilemenu-click{ text, path }
 *  FabMenu      [data-dt-fabmenu] FAB with position, items, tooltip;
 *              dispatches dt:fabmenu-click{ text, value }
 *  Breadcrumb   [data-dt-breadcrumb] trail with separators, aria-current;
 *              dispatches dt:breadcrumb-click{ text, path }
 *  Steps        [data-dt-steps] wizard header + content, linear,
 *              selectedIndex; dispatches dt:steps-change{ index }
 *  Splitter     [data-dt-splitter] h/v panes size/min/max/collapsible,
 *              separator handle; dispatches dt:splitter-resize /
 *              dt:splitter-collapse (cancelable)
 *  Toc          [data-dt-toc] scroll-spy, selector scope, orientation;
 *              dispatches dt:toc-click{ text, selector }
 *  Tree         [data-dt-tree] tree/treeitem with expand/collapse, selection;
 *              dispatches dt:tree-select/expand/collapse
 *  PickList     [data-dt-picklist] source/target listboxes, move/up/down;
 *              dispatches dt:picklist-move{ source, target, moved, direction }
 *  Scheduler    [data-dt-scheduler] grid with slots/events, prev/next, keyboard;
 *              dispatches dt:scheduler-event-click/slot-click/date-change
 *  Gantt        [data-dt-gantt] grid with task bars, progress, dependencies;
 *              dispatches dt:gantt-task-click
 *  Pivot        [data-dt-pivot] field chips dispatch dt:pivot-field-remove
 *  Timeline     [data-dt-timeline] static list (reverse via data-dt-reverse)
 *  VirtualGrid  [data-dt-virtual-grid]
 *  Chart        [data-dt-chart] json series, nice ticks, tooltip, click htmx-driven windowing on reveal
 *  QRCode       [data-dt-qrcode] static server-rendered svg
 *  Barcode      [data-dt-barcode] static server-rendered svg
 *  Carousel     [data-dt-carousel] slides with auto/interval/pauseOnHover,
 *              arrows/indicators; dispatches dt:carousel-change{ index }
...
/* ---------------- Datepicker ---------------- */
  ...
  // [data-dt-pager] is a nav landmark with first/prev/page/next/last buttons,
  // optional summary (aria-live polite) and page-size select. The behavior
  // derives pageCount from data-dt-count / data-dt-page-size, handles
  // alwaysVisible, computes ellipsis items, wires clicks and keyboard, and
  // dispatches dt:page-change{page, skip, top, pageCount, pageSize} and
  // dt:page-size-change{pageSize}. Init is idempotent via _dtPager;
  // window.dtUikit.pager.init(root) forces it.

  function pagerData(root) {
    if (!root._dtPager) {
      root._dtPager = { ready: false };
    }
    return root._dtPager;
  }
  ...
  function pagerRender(root) {
    ...
    if (!st.ready) return;
    const count = Number(root.getAttribute("data-dt-count")) || 0;
    const pageSize = Number(root.getAttribute("data-dt-page-size")) || 10;
    const pageCount = pagerPageCount(count, pageSize);
    const alwaysVisible = root.hasAttribute("data-dt-always-visible");
    if (!alwaysVisible && pageCount <= 1) {
      root.hidden = true;
      return;
    }
    root.hidden = false;
    const raw = Number(root.getAttribute("data-dt-page")) || 1;
    const current = Math.min(Math.max(1, raw), pageCount);
    root.setAttribute("data-dt-page", String(current));
    // summary
    const summary = root.querySelector("[data-dt-pager-summary]");
    if (summary) {
      const show = root.getAttribute("data-dt-show-paging-summary");
      const visible = show === null || show !== "false";
      summary.hidden = !visible;
      if (visible) {
        const fmt = root.getAttribute("data-dt-paging-summary-format") || "Page {0} of {1} ({2} items)";
        summary.textContent = pagerFormat(fmt, current, pageCount, count);
      }
    }
    // buttons disabled + active
    const first = root.querySelector("[data-dt-pager-first]");
    const prev = root.querySelector("[data-dt-pager-prev]");
    const next = root.querySelector("[data-dt-pager-next]");
    const last = root.querySelector("[data-dt-pager-last]");
    if (first) first.disabled = current <= 1;
    if (prev) prev.disabled = current <= 1;
    if (next) next.disabled = current >= pageCount;
    if (last) last.disabled = current >= pageCount;
    root.querySelectorAll("[data-dt-pager-page]").forEach((btn) => {
      const v = Number(btn.getAttribute("data-dt-page"));
      const active = v === current;
      btn.classList.toggle("dt-pager-button--active", active);
      if (active) btn.setAttribute("aria-current", "page");
      else btn.removeAttribute("aria-current");
    });
    // horizontal align class
    const align = root.getAttribute("data-dt-horizontal-align");
    root.classList.remove("dt-pager--left", "dt-pager--center", "dt-pager--right", "dt-pager--justify");
    if (align === "center") root.classList.add("dt-pager--center");
    else if (align === "right") root.classList.add("dt-pager--right");
    else if (align === "justify") root.classList.add("dt-pager--justify");
    else root.classList.add("dt-pager--left");
  }
  ...
  function pagerSetPage(root, page) {
    ...
    const current = Number(root.getAttribute("data-dt-page")) || 1;
    if (next === current) return;
    root.setAttribute("data-dt-page", String(next));
    pagerRender(root);
    const skip = (next - 1) * pageSize;
    root.dispatchEvent(new CustomEvent("dt:page-change", { bubbles: true, detail: { page: next, skip, top: pageSize, pageCount, pageSize } }));
  }
  ...
  on("[data-dt-pager-first]", "click", (btn) => {
    const root = btn.closest("[data-dt-pager]");
    if (!root) return;
    initPager(root);
    if (btn.disabled) return;
    pagerSetPage(root, 1);
  ...
  on("[data-dt-pager-prev]", "click", (btn) => {
    const root = btn.closest("[data-dt-pager]");
    if (!root) return;
    initPager(root);
    if (btn.disabled) return;
  ...
  on("[data-dt-pager-next]", "click", (btn) => {
    const root = btn.closest("[data-dt-pager]");
    if (!root) return;
    initPager(root);
    if (btn.disabled) return;
  ...
  on("[data-dt-pager-last]", "click", (btn) => {
    const root = btn.closest("[data-dt-pager]");
    if (!root) return;
    initPager(root);
    if (btn.disabled) return;
  ...
  on("[data-dt-pager-page]", "click", (btn) => {
    const root = btn.closest("[data-dt-pager]");
    if (!root) return;
    initPager(root);
    if (btn.disabled) return;
    const page = Number(btn.getAttribute("data-dt-page"));
    pagerSetPage(root, page);
  });

  on("[data-dt-pager-size-select]", "change", (select) => {
    const root = select.closest("[data-dt-pager]");
    if (!root) return;
    initPager(root);
    const pageSize = Number(select.value);
    root.dispatchEvent(new CustomEvent("dt:page-size-change", { bubbles: true, detail: { pageSize } }));
  });

  on("[data-dt-pager-controls]", "keydown", (controls, e) => {
    const buttons = [...controls.querySelectorAll("button[data-dt-pager-page], button[data-dt-pager-first], button[data-dt-pager-prev], button[data-dt-pager-next], button[data-dt-pager-last]")].filter(
      (b) => !b.disabled,
    );
  ...
  document.querySelectorAll("[data-dt-pager]").forEach(initPager);

  /* ---------------- Menu ---------------- */

  function menuData(root) {
    if (!root._dtMenu) root._dtMenu = { ready: false };
    return root._dtMenu;
  }

  function menuCloseAll(root) {
    root.querySelectorAll("[data-dt-menu-submenu]").forEach((sm) => (sm.hidden = true));
    root.querySelectorAll("[data-dt-menu-item][aria-haspopup]").forEach((btn) => btn.setAttribute("aria-expanded", "false"));
  }
  ...
  on("[data-dt-menu-item]", "click", (btn) => {
    const root = btn.closest("[data-dt-menu]");
    if (!root) return;
    initMenu(root);
    ...
    if (hasPopup) {
      ...
      if (!submenu || !submenu.hasAttribute("data-dt-menu-submenu")) return;
      // close others
      menuCloseAll(root);
      if (!expanded) {
    ...
    // leaf click
    const text = btn.getAttribute("data-dt-text") || btn.textContent.trim();
    const value = btn.getAttribute("data-dt-value") || undefined;
    const path = btn.getAttribute("data-dt-path") || undefined;
    root.dispatchEvent(new CustomEvent("dt:menu-click", { bubbles: true, detail: { text, value, path } }));
    menuCloseAll(root);
  });

  on("[data-dt-menu-item]", "mouseenter", (btn) => {
    const root = btn.closest("[data-dt-menu]");
    if (!root || root.getAttribute("data-dt-orientation") === "vertical") return;
  ...
  on("[data-dt-menu]", "keydown", (root, e) => {
    initMenu(root);
    const items = [...root.querySelectorAll("[data-dt-menu-item]:not([aria-disabled])")].filter((b) => !b.disabled);
    ...
    if (e.key === "ArrowRight") {
      ...
      if (current && current.getAttribute("aria-haspopup") === "menu") {
        ...
        if (submenu) {
          ...
          submenu.hidden = false;
          const first = submenu.querySelector("[data-dt-menu-item]");
          first?.focus();
        }
      } else if (current && current.closest("[data-dt-menu-submenu]")) {
        e.preventDefault();
        const next = items[idx + 1] ?? items[0];
        next?.focus();
      }
      ...
      menuCloseAll(root);
      const trigger = root.querySelector("[data-dt-menu-item][aria-haspopup][aria-expanded='true']");
      if (trigger) trigger.focus();
    }
  });

  document.addEventListener("click", (e) => {
    if (e.target instanceof Element && !e.target.closest("[data-dt-menu]")) {
      document.querySelectorAll("[data-dt-menu]").forEach(menuCloseAll);
    }
  });

  document.querySelectorAll("[data-dt-menu]").forEach(initMenu);

  /* ---------------- PanelMenu ---------------- */

  function panelMenuData(root) {
    if (!root._dtPanelMenu) root._dtPanelMenu = { ready: false };
    return root._dtPanelMenu;
  }
  ...
  function initPanelMenu(root) {
    ...
    if (st.ready) return;
    st.ready = true;
    const multiple = root.hasAttribute("data-dt-multiple");
    if (!multiple) {
      // ensure only first expanded stays open
      const expanded = root.querySelectorAll("[data-dt-panelmenu-item][aria-expanded='true']");
  ...
  on("[data-dt-panelmenu-item]", "click", (btn) => {
    const root = btn.closest("[data-dt-panelmenu]");
    if (!root) return;
    initPanelMenu(root);
    if (btn.hasAttribute("aria-disabled") || btn.disabled) return;
    ...
    if (hasPopup) {
      ...
      if (!multiple && !expanded) {
        // collapse others
        root.querySelectorAll("[data-dt-panelmenu-item][aria-expanded='true']").forEach((other) => {
          if (other === btn) return;
    ...
    const value = btn.getAttribute("data-dt-value") || undefined;
    const path = btn.getAttribute("data-dt-path") || undefined;
    root.dispatchEvent(new CustomEvent("dt:panelmenu-click", { bubbles: true, detail: { text, value, path } }));
  });

  on("[data-dt-panelmenu]", "keydown", (root, e) => {
    initPanelMenu(root);
    const items = [...root.querySelectorAll("[data-dt-panelmenu-item]:not([aria-disabled])")].filter((b) => !b.disabled);
    const idx = items.indexOf(document.activeElement);
    if (e.key === "ArrowDown") {
      e.preventDefault();
      const next = items[idx + 1] ?? items[0];
      ...
      const expanded = root.querySelector("[data-dt-panelmenu-item][aria-expanded='true']");
      if (expanded) {
  ...
  document.querySelectorAll("[data-dt-panelmenu]").forEach(initPanelMenu);

  /* ---------------- ProfileMenu ---------------- */

  function profileMenuData(root) {
    if (!root._dtProfileMenu) root._dtProfileMenu = { ready: false, open: false };
    return root._dtProfileMenu;
  }

  function profileMenuSetOpen(root, open) {
    const st = profileMenuData(root);
    st.open = open;
    const trigger = root.querySelector("[data-dt-profilemenu-trigger]");
    const menu = root.querySelector("[data-dt-profilemenu-menu]");
    if (trigger) trigger.setAttribute("aria-expanded", String(open));
    if (menu) menu.hidden = !open;
    if (open && menu) {
      const first = menu.querySelector("[data-dt-profilemenu-item]:not([aria-disabled])");
  ...
  on("[data-dt-profilemenu-trigger]", "click", (btn) => {
    const root = btn.closest("[data-dt-profilemenu]");
    if (!root) return;
  ...
  on("[data-dt-profilemenu-item]", "click", (btn) => {
    const root = btn.closest("[data-dt-profilemenu]");
    if (!root) return;
    initProfileMenu(root);
    if (btn.hasAttribute("aria-disabled") || btn.disabled) return;
    const text = btn.getAttribute("data-dt-text") || btn.textContent.trim();
    const path = btn.getAttribute("data-dt-path") || undefined;
    root.dispatchEvent(new CustomEvent("dt:profilemenu-click", { bubbles: true, detail: { text, path } }));
    profileMenuSetOpen(root, false);
    const trigger = root.querySelector("[data-dt-profilemenu-trigger]");
    trigger?.focus();
  });

  on("[data-dt-profilemenu]", "keydown", (root, e) => {
    initProfileMenu(root);
    const st = profileMenuData(root);
    const menu = root.querySelector("[data-dt-profilemenu-menu]");
    if (!menu || menu.hidden) {
      if (e.key === "ArrowDown" || e.key === "Enter" || e.key === " ") {
        ...
        if (trigger && trigger.hasAttribute("data-dt-profilemenu-trigger")) {
          e.preventDefault();
    ...
    const items = [...menu.querySelectorAll("[data-dt-profilemenu-item]:not([aria-disabled])")].filter((b) => !b.disabled);
    const idx = items.indexOf(document.activeElement);
    if (e.key === "ArrowDown") {
      e.preventDefault();
      const next = items[idx + 1] ?? items[0];
  ...
  document.addEventListener("click", (e) => {
    if (e.target instanceof Element && !e.target.closest("[data-dt-profilemenu]")) {
      document.querySelectorAll("[data-dt-profilemenu]").forEach((r) => profileMenuSetOpen(r, false));
    }
  });

  document.querySelectorAll("[data-dt-profilemenu]").forEach(initProfileMenu);

  /* ---------------- FabMenu ---------------- */

  function fabMenuData(root) {
    if (!root._dtFabMenu) root._dtFabMenu = { ready: false, open: false };
    return root._dtFabMenu;
  }

  function fabMenuSetOpen(root, open) {
    const st = fabMenuData(root);
    st.open = open;
    const trigger = root.querySelector("[data-dt-fabmenu-trigger]");
    const menu = root.querySelector("[data-dt-fabmenu-menu]");
    if (trigger) trigger.setAttribute("aria-expanded", String(open));
    if (menu) menu.hidden = !open;
  }
  ...
  on("[data-dt-fabmenu-trigger]", "click", (btn) => {
    const root = btn.closest("[data-dt-fabmenu]");
    if (!root) return;
    initFabMenu(root);
    const st = fabMenuData(root);
  ...
  on("[data-dt-fabmenu-item]", "click", (btn) => {
    const root = btn.closest("[data-dt-fabmenu]");
    if (!root) return;
    initFabMenu(root);
    if (btn.hasAttribute("aria-disabled") || btn.disabled) return;
    const text = btn.getAttribute("data-dt-text") || btn.textContent.trim();
    const value = btn.getAttribute("data-dt-value") || undefined;
    root.dispatchEvent(new CustomEvent("dt:fabmenu-click", { bubbles: true, detail: { text, value } }));
    fabMenuSetOpen(root, false);
  });

  on("[data-dt-fabmenu]", "keydown", (root, e) => {
    initFabMenu(root);
    const st = fabMenuData(root);
  ...
  document.addEventListener("click", (e) => {
    if (e.target instanceof Element && !e.target.closest("[data-dt-fabmenu]")) {
      document.querySelectorAll("[data-dt-fabmenu]").forEach((r) => fabMenuSetOpen(r, false));
    }
  });

  document.querySelectorAll("[data-dt-fabmenu]").forEach(initFabMenu);

  /* ---------------- Breadcrumb ---------------- */

  function breadcrumbData(root) {
    if (!root._dtBreadcrumb) root._dtBreadcrumb = { ready: false };
    return root._dtBreadcrumb;
  }
  ...
  on("[data-dt-breadcrumb-link], [data-dt-breadcrumb-current]", "click", (el) => {
    const root = el.closest("[data-dt-breadcrumb]");
    if (!root) return;
    initBreadcrumb(root);
    ...
    if (el.hasAttribute("data-dt-breadcrumb-current") && !el.hasAttribute("data-dt-path") && !el.getAttribute("href")) return;
    const text = el.getAttribute("data-dt-text") || el.textContent.trim();
    const path = el.getAttribute("data-dt-path") || el.getAttribute("href") || undefined;
    // allow default link navigation, but also dispatch event
    root.dispatchEvent(new CustomEvent("dt:breadcrumb-click", { bubbles: true, detail: { text, path } }));
  });

  on("[data-dt-breadcrumb]", "keydown", (root, e) => {
    initBreadcrumb(root);
    if (e.key === "Enter" || e.key === " ") {
      const target = e.target;
      if (target instanceof Element && (target.hasAttribute("data-dt-breadcrumb-link") || target.hasAttribute("data-dt-breadcrumb-current"))) {
  ...
  document.querySelectorAll("[data-dt-breadcrumb]").forEach(initBreadcrumb);

  /* ---------------- Steps ---------------- */

  function stepsData(root) {
    if (!root._dtSteps) root._dtSteps = { ready: false, index: 0 };
    return root._dtSteps;
  }

  function stepsItems(root) {
    return [...root.querySelectorAll("[data-dt-steps-item]")];
  }

  function stepsPanels(root) {
    return [...root.querySelectorAll("[data-dt-steps-panel]")];
  }

  function stepsIsLinear(root) {
    return root.hasAttribute("data-dt-linear");
  }

  function stepsConnector(li, completed) {
    const c = li.querySelector(".dt-steps-connector");
    if (c) c.classList.toggle("dt-steps-connector--completed", completed);
  }
  ...
  function stepsRender(root) {
    ...
    items.forEach((item, i) => {
      const li = item.closest(".dt-steps-item") || item.closest("li") || item.parentElement;
      const isActive = i === active;
      const isCompleted = i < active;
      let isDisabled = item.hasAttribute("aria-disabled") || item.disabled || item.getAttribute("disabled") !== null;
      if (!isDisabled && item.hasAttribute("data-dt-disabled")) isDisabled = true;
      if (!isDisabled && linear && i > active + 1) isDisabled = true;
      const textDisabled = item.getAttribute("data-dt-text");
      void textDisabled;
      item.classList.toggle("dt-steps-step--active", isActive);
      item.classList.toggle("dt-steps-step--completed", isCompleted);
      item.classList.toggle("dt-steps-step--disabled", isDisabled);
      ...
      const circle = item.querySelector(".dt-steps-circle");
      if (circle) {
        const icon = item.getAttribute("data-dt-icon");
        let html = "";
        if (isCompleted) {
          html = '<span class="dt-steps-check" aria-hidden="true">✓</span>';
        } else if (icon) {
          html = `<span class="dt-steps-icon" aria-hidden="true">${icon}</span>`;
        } else {
          html = `<span class="dt-steps-number" aria-hidden="true">${i + 1}</span>`;
        }
    ...
    panels.forEach((panel) => {
      const idx = Number(panel.getAttribute("data-dt-index"));
      panel.hidden = idx !== active;
      if (idx === active) panel.removeAttribute("hidden");
      else panel.setAttribute("hidden", "");
    });
    root.setAttribute("data-dt-selected-index", String(active));
  }
  ...
  function stepsSetActive(root, index) {
    ...
    st.index = clamped;
    stepsRender(root);
    root.dispatchEvent(new CustomEvent("dt:steps-change", { bubbles: true, detail: { index: clamped } }));
  }
  ...
  function initSteps(root) {
    ...
    const items = stepsItems(root);
    if (items.length === 0) return;
    let active = Number(root.getAttribute("data-dt-selected-index"));
  ...
  on("[data-dt-steps-item]", "click", (btn) => {
    const root = btn.closest("[data-dt-steps]");
    if (!root) return;
    initSteps(root);
    const st = stepsData(root);
    const items = stepsItems(root);
    const idxAttr = btn.getAttribute("data-dt-index");
  ...
  on("[data-dt-steps]", "keydown", (root, e) => {
    initSteps(root);
    const st = stepsData(root);
    ...
    if (e.key === "ArrowRight" || e.key === "ArrowDown") {
      ...
      const target = e.target;
      if (target instanceof Element && target.hasAttribute("data-dt-steps-item")) {
        e.preventDefault();
  ...
  document.querySelectorAll("[data-dt-steps]").forEach(initSteps);
  ...
  function splitterData(root) {
    if (!root._dtSplitter) root._dtSplitter = { ready: false, sizes: [], collapsed: [], prevSizes: [], dragging: null };
    return root._dtSplitter;
  }

  function splitterPanes(root) {
    return [...root.querySelectorAll("[data-dt-splitter-pane]")];
  }

  function splitterHandles(root) {
    return [...root.querySelectorAll("[data-dt-splitter-handle]")];
  }

  function splitterIsHorizontal(root) {
    const orient = root.getAttribute("data-dt-orientation") || root.getAttribute("data-dt-direction") || "";
    if (orient === "vertical") return false;
    if (orient === "horizontal") return true;
    return !root.classList.contains("dt-splitter--vertical");
  }

  function splitterEmitResize(root, paneIndex, newSize) {
    const detail = { paneIndex, newSize, cancel: false };
    root.dispatchEvent(new CustomEvent("dt:splitter-resize", { bubbles: true, cancelable: true, detail }));
    return !detail.cancel;
  }

  function splitterEmitCollapse(root, paneIndex, collapse) {
    const detail = { paneIndex, collapse, cancel: false };
    root.dispatchEvent(new CustomEvent("dt:splitter-collapse", { bubbles: true, cancelable: true, detail }));
    return !detail.cancel;
  }
  ...
  function splitterRender(root) {
    ...
    panes.forEach((pane, i) => {
      ...
      pane.classList.toggle("dt-splitter-pane--collapsed", isCollapsed);
      if (isCollapsed) pane.setAttribute("data-dt-collapsed", "");
      else pane.removeAttribute("data-dt-collapsed");
      pane.setAttribute("aria-label", pane.getAttribute("aria-label") || `Pane ${i + 1}`);
      const btn = pane.querySelector("[data-dt-splitter-collapse]");
      if (btn) {
    ...
    root.querySelectorAll("[data-dt-splitter-collapse]").forEach((btn) => {
      const idx = Number(btn.getAttribute("data-dt-index"));
      if (!Number.isNaN(idx) && st.collapsed[idx] !== undefined) {
        ...
        if (panes[idx] && panes[idx].style.display === "none") {
          btn.classList.add("dt-splitter-collapse--outside");
        } else {
          btn.classList.remove("dt-splitter-collapse--outside");
        }
      }
    });
    handles.forEach((handle) => {
      const idx = Number(handle.getAttribute("data-dt-index"));
      const size = st.sizes[idx] ?? 0;
      const pane = panes[idx];
      const min = pane ? splitterParsePercent(pane.getAttribute("data-dt-min"), 0) : 0;
      const max = pane ? splitterParsePercent(pane.getAttribute("data-dt-max"), 100) : 100;
  ...
  function splitterToggleCollapse(root, paneIndex) {
    ...
    const collapsible = pane && pane.hasAttribute("data-dt-collapsible");
    if (!collapsible) {
  ...
  function initSplitter(root) {
    ...
    const panes = splitterPanes(root);
    const count = panes.length;
    st.collapsed = panes.map((p) => p.hasAttribute("data-dt-collapsed"));
    const raw = panes.map((p, i) => {
      const sizeAttr = p.getAttribute("data-dt-size");
  ...
  on("[data-dt-splitter-handle]", "pointerdown", (handle, e) => {
    const root = handle.closest("[data-dt-splitter]");
    if (!root) return;
    initSplitter(root);
    const st = splitterData(root);
    const idx = Number(handle.getAttribute("data-dt-index"));
    ...
    const onMove = (ev) => {
      ...
      const panes = splitterPanes(root);
      const mins = panes.map((p) => splitterParsePercent(p.getAttribute("data-dt-min"), 0));
      const maxs = panes.map((p) => splitterParsePercent(p.getAttribute("data-dt-max"), 100));
  ...
  on("[data-dt-splitter-collapse]", "click", (btn) => {
    const root = btn.closest("[data-dt-splitter]");
    if (!root) return;
    initSplitter(root);
    const idx = Number(btn.getAttribute("data-dt-index"));
    if (Number.isNaN(idx)) return;
  ...
  on("[data-dt-splitter-handle]", "keydown", (handle, e) => {
    const root = handle.closest("[data-dt-splitter]");
    if (!root) return;
    initSplitter(root);
    const st = splitterData(root);
    ...
    const oldRight = st.sizes[idx + 1] ?? 0;
    const total = oldLeft + oldRight;
    const leftCollapsible = panes[idx]?.hasAttribute("data-dt-collapsible");
    const rightCollapsible = panes[idx + 1]?.hasAttribute("data-dt-collapsible");
  ...
  document.querySelectorAll("[data-dt-splitter]").forEach(initSplitter);

  /* ---------------- Toc ---------------- */

  function tocData(root) {
    if (!root._dtToc) root._dtToc = { ready: false, active: null, observer: null, handler: null };
    return root._dtToc;
  }

  function tocItems(root) {
    return [...root.querySelectorAll("[data-dt-toc-item]")];
  }

  function tocScope(root) {
    const sel = root.getAttribute("data-dt-selector") || root.getAttribute("data-dt-toc-selector") || root.getAttribute("data-dt-scope");
    if (sel) {
  ...
  function tocSetActive(root, selector) {
    ...
    items.forEach((link) => {
      const sel = link.getAttribute("data-dt-selector") || link.getAttribute("href") || "";
      const normalized = sel.startsWith("#") || sel.startsWith(".") ? sel : `#${sel}`;
      const targetSel = link.getAttribute("data-dt-selector") || link.getAttribute("href");
      const isActive = targetSel === selector || normalized === selector || link.getAttribute("data-dt-selector") === selector;
      ...
      if (active) {
        ...
        link.classList.add("dt-toc-link--active");
      } else {
        link.removeAttribute("aria-current");
        link.classList.remove("dt-toc-link--active");
      }
    });
  }

  function tocHandleSelect(root, item) {
    const selector = item.getAttribute("data-dt-selector") || item.getAttribute("href") || "";
    const text = item.getAttribute("data-dt-text") || item.textContent.trim();
    tocSetActive(root, selector);
    root.dispatchEvent(new CustomEvent("dt:toc-click", { bubbles: true, detail: { text, selector } }));
  ...
  function initToc(root) {
    ...
    const initialSelector = initial ? (initial.getAttribute("data-dt-selector") || initial.getAttribute("href")) : items[0].getAttribute("data-dt-selector") || items[0].getAttribute("href");
    ...
    const orient = root.getAttribute("data-dt-orientation") || "vertical";
    root.classList.toggle("dt-toc--vertical", orient === "vertical");
    root.classList.toggle("dt-toc--horizontal", orient === "horizontal");
  }

  on("[data-dt-toc-item]", "click", (link, e) => {
    const root = link.closest("[data-dt-toc]");
    if (!root) return;
    initToc(root);
    e.preventDefault();
  ...
  document.querySelectorAll("[data-dt-toc]").forEach(initToc);

  /* ---------------- Carousel ---------------- */

  function carouselData(root) {
    if (!root._dtCarousel) root._dtCarousel = { ready: false, index: 0, timer: null, manualPaused: false, hoverPaused: false };
    return root._dtCarousel;
  }

  function carouselSlides(root) {
    return [...root.querySelectorAll("[data-dt-carousel-item]")];
  }

  function carouselIndicators(root) {
    return [...root.querySelectorAll("[data-dt-carousel-indicator]")];
  }

  function carouselIsAuto(root) {
    return root.hasAttribute("data-dt-auto");
  }

  function carouselInterval(root) {
    const raw = root.getAttribute("data-dt-interval");
    const n = raw ? Number(raw) : 3000;
    return Number.isFinite(n) && n > 0 ? n : 3000;
  }

  function carouselPauseOnHover(root) {
    const val = root.getAttribute("data-dt-pause-on-hover");
  ...
  function carouselRender(root) {
    ...
    slides.forEach((slide, i) => {
      const isActive = i === active;
      slide.classList.toggle("dt-carousel-slide--active", isActive);
      if (isActive) {
    ...
    indicators.forEach((btn, i) => {
      const isActive = i === active;
      btn.classList.toggle("dt-carousel-indicator--active", isActive);
      if (isActive) btn.setAttribute("aria-current", "true");
      else btn.removeAttribute("aria-current");
      btn.setAttribute("aria-label", `Go to slide ${i + 1}`);
      btn.tabIndex = 0;
    });
    const pauseBtn = root.querySelector("[data-dt-carousel-pause]");
    if (pauseBtn) {
    ...
    const showArrowsAttr = root.getAttribute("data-dt-show-arrows");
    const showIndicatorsAttr = root.getAttribute("data-dt-show-indicators");
    const showArrows = showArrowsAttr === "false" ? false : true;
    const showIndicators = showIndicatorsAttr === "false" ? false : true;
    root.querySelectorAll("[data-dt-carousel-prev], [data-dt-carousel-next]").forEach((el) => {
      el.hidden = !showArrows || total <= 1;
    });
    const indContainer = root.querySelector("[data-dt-carousel-indicators]");
  ...
  function carouselSetIndex(root, next) {
    ...
    root.dispatchEvent(new CustomEvent("dt:carousel-change", { bubbles: true, detail: { index: clamped } }));
  }
  ...
  function initCarousel(root) {
    ...
    const slides = carouselSlides(root);
    if (slides.length === 0) return;
    let idx = Number(root.getAttribute("data-dt-selected-index"));
    if (Number.isNaN(idx) || !Number.isFinite(idx)) {
      const activeIdx = slides.findIndex((s) => s.classList.contains("dt-carousel-slide--active") || !s.hasAttribute("hidden"));
  ...
  on("[data-dt-carousel-prev]", "click", (btn) => {
    const root = btn.closest("[data-dt-carousel]");
    if (!root) return;
    initCarousel(root);
    carouselPrev(root);
  });

  on("[data-dt-carousel-next]", "click", (btn) => {
    const root = btn.closest("[data-dt-carousel]");
    if (!root) return;
    initCarousel(root);
    carouselNext(root);
  ...
  on("[data-dt-carousel-indicator]", "click", (btn) => {
    const root = btn.closest("[data-dt-carousel]");
    if (!root) return;
    initCarousel(root);
    const idx = Number(btn.getAttribute("data-dt-index"));
  ...
  on("[data-dt-carousel-pause]", "click", (btn) => {
    const root = btn.closest("[data-dt-carousel]");
    if (!root) return;
    initCarousel(root);
    const st = carouselData(root);
  ...
  on("[data-dt-carousel]", "keydown", (root, e) => {
    initCarousel(root);
    const slides = carouselSlides(root);
    if (slides.length === 0) return;
    if (e.key === "ArrowLeft") {
  ...
  on("[data-dt-carousel]", "mouseenter", (root) => {
  ...
  on("[data-dt-carousel]", "mouseleave", (root) => {
    if (!carouselIsAuto(root) || !carouselPauseOnHover(root)) return;
    const st = carouselData(root);
    st.hoverPaused = false;
    carouselUpdateTimer(root);
  ...
  on("[data-dt-carousel]", "focusin", (root) => {
    if (!carouselIsAuto(root) || !carouselPauseOnHover(root)) return;
    const st = carouselData(root);
    st.hoverPaused = true;
    carouselUpdateTimer(root);
  ...
  on("[data-dt-carousel]", "focusout", (root, e) => {
    if (!carouselIsAuto(root) || !carouselPauseOnHover(root)) return;
  ...
  document.querySelectorAll("[data-dt-carousel]").forEach(initCarousel);

  /* ---------------- Tree ---------------- */

  function treeData(root) {
    if (!root._dtTree) root._dtTree = { ready: false };
    return root._dtTree;
  }
  ...
  on("[data-dt-tree-caret]", "click", (btn) => {
    const item = btn.closest("[data-dt-tree-item]");
    const root = btn.closest("[data-dt-tree]");
    if (!item || !root) return;
    initTree(root);
    const expanded = item.getAttribute("aria-expanded") === "true";
    const key = item.getAttribute("data-dt-key");
    const children = item.querySelector(":scope > [data-dt-tree-children]");
    if (children) children.hidden = expanded;
    item.setAttribute("aria-expanded", String(!expanded));
    btn.setAttribute("aria-expanded", String(!expanded));
    btn.setAttribute("aria-label", `${!expanded ? "Collapse" : "Expand"} ${item.getAttribute("data-dt-text") || ""}`.trim());
    const detail = { key, text: item.getAttribute("data-dt-text") };
    root.dispatchEvent(new CustomEvent(expanded ? "dt:tree-collapse" : "dt:tree-expand", { bubbles: true, detail }));
  });

  on("[data-dt-tree-item]", "click", (item) => {
    const root = item.closest("[data-dt-tree]");
    if (!root || !item.hasAttribute("data-dt-tree-item")) return;
    initTree(root);
    if (item.hasAttribute("aria-disabled") || item.getAttribute("data-dt-disabled") !== null) return;
    // if click was on caret, already handled
    if (item.querySelector(":scope > [data-dt-tree-caret]") && item.contains(document.activeElement) && document.activeElement.hasAttribute("data-dt-tree-caret")) return;
    const key = item.getAttribute("data-dt-key");
    const text = item.getAttribute("data-dt-text") || item.textContent.trim();
    const selected = item.getAttribute("aria-selected") === "true";
    const mode = root.getAttribute("data-dt-selection-mode") || "single";
    if (mode === "multiple") {
      item.setAttribute("aria-selected", String(!selected));
    } else {
      root.querySelectorAll("[data-dt-tree-item][aria-selected='true']").forEach((el) => el.setAttribute("aria-selected", "false"));
      item.setAttribute("aria-selected", "true");
    }
    root.dispatchEvent(new CustomEvent("dt:tree-select", { bubbles: true, detail: { key, text, selected: !selected } }));
  });

  on("[data-dt-tree]", "keydown", (root, e) => {
    initTree(root);
    const items = [...root.querySelectorAll("[data-dt-tree-item]:not([aria-disabled])")].filter((el) => el.getAttribute("data-dt-disabled") === null);
    const idx = items.indexOf(document.activeElement);
    if (e.key === "ArrowDown") {
      e.preventDefault();
      const next = items[idx + 1] ?? items[0];
      ...
      if (cur && cur.getAttribute("aria-expanded") === "false") {
        const caret = cur.querySelector("[data-dt-tree-caret]");
        caret?.click();
      } else if (cur) {
        const child = cur.querySelector(":scope > [data-dt-tree-children] [data-dt-tree-item]");
        child?.focus();
      }
    } else if (e.key === "ArrowLeft") {
      const cur = document.activeElement;
      if (cur && cur.getAttribute("aria-expanded") === "true") {
        ...
        const parent = cur?.closest("[data-dt-tree-children]")?.closest("[data-dt-tree-item]");
        parent?.focus();
      }
    } else if (e.key === "Home") {
      e.preventDefault();
  ...
  document.querySelectorAll("[data-dt-tree]").forEach(initTree);

  /* ---------------- PickList ---------------- */

  function pickListData(root) {
    if (!root._dtPickList) root._dtPickList = { ready: false };
    return root._dtPickList;
  }

  function pickListSelectedKeys(list) {
    return [...list.querySelectorAll("[data-dt-picklist-item][aria-selected='true']")].map((el) => el.getAttribute("data-dt-key"));
  }
  ...
  function pickListMove(root, direction) {
    const sourceList = root.querySelector('[data-dt-side="source"]');
    const targetList = root.querySelector('[data-dt-side="target"]');
    if (!sourceList || !targetList) return;
    let moved = [];
    let sourceItems = [...sourceList.querySelectorAll("[data-dt-picklist-item]")];
    let targetItems = [...targetList.querySelectorAll("[data-dt-picklist-item]")];
    if (direction === "toTarget") {
      moved = sourceList.querySelectorAll("[data-dt-picklist-item][aria-selected='true']:not([aria-disabled])");
      moved.forEach((el) => targetList.appendChild(el));
    } else if (direction === "toSource") {
      moved = targetList.querySelectorAll("[data-dt-picklist-item][aria-selected='true']:not([aria-disabled])");
      moved.forEach((el) => sourceList.appendChild(el));
    } else if (direction === "allToTarget") {
      moved = sourceList.querySelectorAll("[data-dt-picklist-item]:not([aria-disabled])");
      moved.forEach((el) => targetList.appendChild(el));
    } else if (direction === "allToSource") {
      moved = targetList.querySelectorAll("[data-dt-picklist-item]:not([aria-disabled])");
      moved.forEach((el) => sourceList.appendChild(el));
    } else if (direction === "up" || direction === "down") {
      const list = root.querySelector('[data-dt-side="target"]');
      const selected = [...list.querySelectorAll("[data-dt-picklist-item][aria-selected='true']")];
    ...
    // clear selection
    root.querySelectorAll("[data-dt-picklist-item][aria-selected='true']").forEach((el) => el.setAttribute("aria-selected", "false"));
    const source = [...sourceList.querySelectorAll("[data-dt-picklist-item]")].map((el) => ({ key: el.getAttribute("data-dt-key"), text: el.getAttribute("data-dt-text") }));
    const target = [...targetList.querySelectorAll("[data-dt-picklist-item]")].map((el) => ({ key: el.getAttribute("data-dt-key"), text: el.getAttribute("data-dt-text") }));
    moved = [...moved].map((el) => ({ key: el.getAttribute("data-dt-key"), text: el.getAttribute("data-dt-text") }));
    root.dispatchEvent(new CustomEvent("dt:picklist-move", { bubbles: true, detail: { source, target, moved, direction } }));
  }

  on("[data-dt-picklist-item]", "click", (item) => {
    const root = item.closest("[data-dt-picklist]");
  ...
  on("[data-dt-picklist-to-target]", "click", (btn) => {
    const root = btn.closest("[data-dt-picklist]");
    if (!root) return;
    initPickList(root);
    pickListMove(root, "toTarget");
  });

  on("[data-dt-picklist-to-source]", "click", (btn) => {
    const root = btn.closest("[data-dt-picklist]");
    if (!root) return;
    initPickList(root);
  ...
  on("[data-dt-picklist-all-to-target]", "click", (btn) => {
    const root = btn.closest("[data-dt-picklist]");
    if (!root) return;
    initPickList(root);
  ...
  on("[data-dt-picklist-all-to-source]", "click", (btn) => {
    const root = btn.closest("[data-dt-picklist]");
    if (!root) return;
    initPickList(root);
  ...
  on("[data-dt-picklist-up]", "click", (btn) => {
    const root = btn.closest("[data-dt-picklist]");
    if (!root) return;
    initPickList(root);
  ...
  on("[data-dt-picklist-down]", "click", (btn) => {
    const root = btn.closest("[data-dt-picklist]");
    if (!root) return;
    initPickList(root);
  ...
  on("[data-dt-picklist]", "keydown", (root, e) => {
    initPickList(root);
    const activeList = document.activeElement?.closest("[data-dt-picklist-list]");
    if (!activeList) return;
    const items = [...activeList.querySelectorAll("[data-dt-picklist-item]:not([aria-disabled])")];
    const idx = items.indexOf(document.activeElement);
    if (e.key === "ArrowDown") {
      e.preventDefault();
      const next = items[idx + 1] ?? items[0];
  ...
  document.querySelectorAll("[data-dt-picklist]").forEach(initPickList);

  /* ---------------- Scheduler ---------------- */

  function schedulerData(root) {
    if (!root._dtScheduler) root._dtScheduler = { ready: false };
    return root._dtScheduler;
  }
  ...
  on("[data-dt-scheduler-prev]", "click", (btn) => {
    const root = btn.closest("[data-dt-scheduler]");
    if (!root) return;
    initScheduler(root);
    root.dispatchEvent(new CustomEvent("dt:scheduler-date-change", { bubbles: true, detail: { dir: -1 } }));
  });
  on("[data-dt-scheduler-next]", "click", (btn) => {
    const root = btn.closest("[data-dt-scheduler]");
    if (!root) return;
    initScheduler(root);
    root.dispatchEvent(new CustomEvent("dt:scheduler-date-change", { bubbles: true, detail: { dir: 1 } }));
  });
  on("[data-dt-scheduler-event]", "click", (el) => {
    const root = el.closest("[data-dt-scheduler]");
    if (!root) return;
    initScheduler(root);
    const id = el.getAttribute("data-dt-id");
    root.dispatchEvent(new CustomEvent("dt:scheduler-event-click", { bubbles: true, detail: { id } }));
  });
  on("[data-dt-scheduler-slot]", "click", (el) => {
    const root = el.closest("[data-dt-scheduler]");
    if (!root) return;
    initScheduler(root);
    root.dispatchEvent(new CustomEvent("dt:scheduler-slot-click", { bubbles: true, detail: {} }));
  });
  document.querySelectorAll("[data-dt-scheduler]").forEach(initScheduler);

  /* ---------------- Gantt ---------------- */

  function ganttData(root) {
    if (!root._dtGantt) root._dtGantt = { ready: false };
    return root._dtGantt;
  }
  ...
  on("[data-dt-gantt-bar]", "click", (el) => {
    const root = el.closest("[data-dt-gantt]");
    if (!root) return;
    initGantt(root);
    const id = el.getAttribute("data-dt-id");
    root.dispatchEvent(new CustomEvent("dt:gantt-task-click", { bubbles: true, detail: { id } }));
  });
  document.querySelectorAll("[data-dt-gantt]").forEach(initGantt);

  /* ---------------- Chart ---------------- */

  function chartData(root) {
    if (!root._dtChart) root._dtChart = { ready: false };
    return root._dtChart;
  }
  function initChart(root) {
    const st = chartData(root);
    if (st.ready) return;
    st.ready = true;
    // htmx chart is server-rendered reference; behavior only wires tooltips + click from data-dt-series JSON if present
    try {
      const raw = root.getAttribute("data-dt-series");
      if (!raw) return;
      JSON.parse(raw);
    } catch {}
  }
  on("[data-dt-chart] [data-dt-chart-click]", "click", (el) => {
    const root = el.closest("[data-dt-chart]");
    if (!root) return;
    initChart(root);
    root.dispatchEvent(new CustomEvent("dt:chart-point-click", { bubbles: true, detail: { seriesTitle: el.getAttribute("data-dt-series-title") || "", category: el.getAttribute("data-dt-category") || "", value: Number(el.getAttribute("data-dt-value") || 0) } }));
  });
  document.querySelectorAll("[data-dt-chart]").forEach(initChart);

  /* ---------------- Pivot ---------------- */

  on("[data-dt-pivot-chip]", "click", (chip) => {
    const root = chip.closest("[data-dt-pivot]");
    if (!root) return;
    root.dispatchEvent(new CustomEvent("dt:pivot-field-remove", {
      bubbles: true,
      detail: { zone: chip.getAttribute("data-dt-zone"), property: chip.getAttribute("data-dt-property") },
    }));
  });

  document.querySelectorAll("[data-dt-pivot]").forEach((root) => { root._dtPivot = { ready: true }; });
