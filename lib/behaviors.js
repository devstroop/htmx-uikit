...
(function () {
  ...
  // [data-dt-field] elements; invalid = aria-invalid="true" or the
  // data-dt-invalid attribute (set by validators — specs/components/
  // validators.md). A blocked submit dispatches dt:invalid with the
  // offending fields; a valid submit dispatches dt:submit with the
  // serialized FormData and proceeds natively.
  ...
  on("[data-dt-form]", "submit", (form, e) => {
    ...
    const invalid = fields.filter(
      (f) => f.getAttribute("aria-invalid") === "true" || f.hasAttribute("data-dt-invalid"),
    );
    if (invalid.length === 0) {
      form.dispatchEvent(
    ...
    form.dispatchEvent(
      new CustomEvent("dt:invalid", {
        bubbles: true,
        detail: {
          fields: invalid.map((f) => ({ name: f.getAttribute("name"), element: f })),
        },
      }),
    );
  });
